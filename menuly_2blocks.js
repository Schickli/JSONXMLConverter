...
Blockly.JSON.toWorkspace = function (json_text, workspace) {
  ...
  startBlock.initSvg();
  startBlock.render();
  Blockly.JSON.buildAndConnect(
    json_structure,
    startBlock.getInput("json").connection
  );
};
...
Blockly.JSON.buildAndConnect = function (json_structure, parentConnection) {
  if (json_structure === null) {
    ...
    var targetBlock = Blockly.Block.obtain(
      parentConnection.sourceBlock_.workspace,
      type
    );
    targetBlock.initSvg();
    targetBlock.render();
    ...
    switch (type) {
      ...
      case "dictionary":
        ...
        for (var key in json_structure) {
          ...
          var elementConnection = targetBlock.getInput(
            "element_" + i
          ).connection;
